...
이미테이션 처치 교수(함수형) - 튜링 제자() test( param ) -> str: return ''

setName ( param ): self.name = param getName () -> str : return self.name
...
Functional Expression case 4 return, param F, F Runable : 무한동력 에너지 - 존재하지 않음 F, T Consumer : setter T, F Supplier : getter T, T Function : 함수로 정의된 것은 표기하지 않아도 반드시 return 이 존재한다. return 이라는 키워드가 없어도 return 은 존재한다. function 이라고 정의한 경우에는 lambda함수는 함수다.
